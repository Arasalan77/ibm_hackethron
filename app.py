...
#     pdf_path = "./demo_patient_data.pdf"
#     # script_dir = os.path.dirname(os.path.abspath(__file__))
#     # pdf_path = os.path.join(script_dir, "demo_patient_data.pdf")
#     pdf_path = "/home/wellness/Arasalan 2024-11-30/codes/discharge_summaries/google_flash_image_summary/nXP-IP2500700.pdf"
...
# Retrieve credentials and prompt templates from environment variables
IBM_URL = os.getenv("IBM_URL")
IBM_APIKEY = os.getenv("IBM_APIKEY")
IBM_PROJECT_ID = os.getenv("IBM_PROJECT_ID")
...
def generate_report(llm, prompt_template, data):
    """
    Generate a report using the provided LLM, prompt template, and input data.
    The data (JSON from the frontend) is injected into the prompt template.
    """
    prompt = prompt_template.format(data=json.dumps(data, indent=2))
    message = HumanMessage(content=[{"type": "text", "text": prompt}])
    response = llm.invoke([message])
...
def generate_reports():
    """
    API endpoint to receive JSON data from the frontend.
    The data is passed to the LLM to generate:
      - A discharge summary
      - Clinical errors report
      - A case summary
    The generated reports are returned as a JSON response.
    """
    print("in generate_reports")
    try:
        input_data = request.get_json()  # Retrieve JSON payload from the request
        ...
        # # Generate reports using the IBM Watsonx LLM
        # discharge_summary = generate_report(ibm_llm, DISCHARGE_SUMMARY_PROMPT_TEMPLATE, input_data)
        # clinical_errors = generate_report(ibm_llm, CLINICAL_ERRORS_PROMPT_TEMPLATE, input_data)
        # case_summary = generate_report(ibm_llm, CASE_SUMMARY_PROMPT_TEMPLATE, input_data)
        ...
        return jsonify("backend api responded"), 200
    except Exception as e:
        logging.exception("Error generating reports")
        return jsonify({"error": str(e)}), 500

if __name__ == "__main__":
    # Run the Flask app (debug mode can be turned off in production)
